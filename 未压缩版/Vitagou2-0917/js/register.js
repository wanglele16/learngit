...
$(function () {
    ...
    $('.sub_btn').click(function(){
        ...
        var Yzm = $('.yzm').val();
        var Yzm2 = $('.yzm2').val();
        if(checkFormByPhone(Userphone, Yzm, Yzm2)){
    ...
    function checkFormByPhone(Userphone, Yzm, Yzm2){
        var reg = /^0?1[3|4|5|8|7][0-9]\d{8}$/;
        if(Userphone != null && Userphone != ''){
            if(!reg.test(Userphone)){
                $('.infor_xf').text('手机号码格式不正确！').show();
        ...
        if(Yzm != null && Yzm != ''){
            if(!patrn.test(Yzm)){
                ...
                if (Yzm2 != Yzm) {
                    $('.infor_xf').text('两次密码不一致！').show();
                    $('.yzm2').css('border-color','#e41e4d');
                    return false;
                } else {
                    //通过基础验证
                    $('.infor_xf').hide();
                    $('.yzm').css('border-color','#03d103');
                    $('.yzm2').css('border-color','#03d103');
                }
